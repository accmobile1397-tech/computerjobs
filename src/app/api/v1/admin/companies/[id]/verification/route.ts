...
import { adminCompanyVerificationSchema } from "@/modules/companies/validators/company.schema";
...
export async function PATCH(
  ...
  try {
    ...
    const body = adminCompanyVerificationSchema.parse(await request.json());
    const company = await updateCompanyVerification({
      companyId: id,
      status: body.status,
      adminUserId,
      ipAddress,
