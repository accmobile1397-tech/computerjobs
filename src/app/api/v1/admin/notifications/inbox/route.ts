...
import { listInboxAdminQuerySchema } from "@/modules/notifications/validators/admin-api.schema";

/**
 * Read-only inbox viewer (C-009-6).
 * GET only — no mark-read / soft-delete / mutation routes under admin inbox.
 */
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const data = await listInboxAdminReadOnly(query);
