...
import { matchApplicationForEmployer } from "@/modules/search/services/match.service";
...
export async function GET(request: NextRequest, context: Ctx) {
  ...
  try {
    await requirePermission(userId, "match:read:employer");
    const result = await matchApplicationForEmployer({
      jobId: id,
      applicationId,
