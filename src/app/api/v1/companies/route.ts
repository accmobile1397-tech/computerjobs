...
import { createCompanySchema } from "@/modules/companies/validators/company.schema";
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const company = await createCompany({ userId, input: body, ipAddress, userAgent });
    return NextResponse.json(successResponse(company, requestId), { status: 201 });
  } catch (error) {
